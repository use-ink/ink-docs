...
Contract references refer to structs which are generated by the ink! codegen for the
...
1. Prep `OtherContract` to be imported to other contracts
...
Next, We to add a way to instantiate `OtherContract`. We do this from the constructor our
of contract.
...
You will need the `substrate-contracts-node` running in the background for the next
steps.
...
[previous section TODO](/basics/cross-contract-calling#contract-references).
