...
`pallet-revive` supports contracts written in Rust and Solidity. For the PolkaVM (PVM) part of the pallet, Solidity is supported by compiling the source code with Parity's [`revive`](https://github.com/paritytech/revive) compiler.
